...
z2=heart_full$z2
p <- ggplot(heart_full, 
            aes(x, y, color = z1, size = z2,axis=FALSE)) + 
  geom_point(pch = -1 * as.hexmode(9829),show.legend = FALSE) + # 9829 utf-8 heart symbol
  #geom_text(label="\u2708",family = "Arial Unicode MS")+
  #geom_emoji('maple_leaf',x=x,y=y,color="orange",size=as.integer(z2*10))+
